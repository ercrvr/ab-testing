...
export default function VideoPlayer({ group }: VideoPlayerProps) {
  ...
  const startSync = useCallback(() => {
    ...
    const tick = () => {
      ...
      const master = videoRefs.current[masterRef.current];
  ...
  const handlePlay = useCallback(
    (sourceIndex: number) => {
      ...
      const currentMaster =
        masterRef.current !== null ? videoRefs.current[masterRef.current] : null;
      ...
      <div className={`grid ${gridCols} gap-4`}>
        {variants.map(([variantName, file], index) => (
          <div key={variantName} className="border border-base-300 rounded-box overflow-hidden">
            ...
            {/* Video player — no onTimeUpdate (polling replaces it) */}
            <div className="p-2 bg-base-300/30">
              <video
              ...
              <div className="mt-1 text-xs text-base-content/50 text-center">
                {file.path.split('/').pop()}
              </div>
            </div>
          </div>
